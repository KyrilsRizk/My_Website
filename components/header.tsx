...
export default function Header() {
 const {activeSection , setActiveSection , setTimeOfLastClick} = useActiveSectionContext();
  return (
    ...
    <motion.div className="flex fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-60 bg-white bg-opacity-80 shadow-lg
     shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[40rem] sm:rounded-full "
            ...
            <Link className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition",{
              ...
              {link.name} 
             {
              link.name === activeSection &&(
                <motion.span className="absolute bg-slate-200 rounded-full inset-0 -z-10"
